...
import static mirai.func.sakura.character.player.Player.DianType;
...
public class Process extends FuncProcess {
    ...
    public void menu() {
        msgEvent.send("-===\uD83C\uDF38落樱之都\uD83C\uDF38===-\n" +
                ...
                "发送◇玩法◇查看游戏各功能的内容");
        sleep(300);
        msgEvent.send("标识说明：[g]群聊可用 [p]私聊可用 [a]仅Bot管理员可用\n" +
                "[gp]注册xxx：注册\n" +
                "[gp]改名xxx：改名\n" +
                "[gp]个人信息\n" +
                "[a][gp]加经验xxx：获取经验\n" +
    ...
    public boolean process() {
        if (!msgEvent.getPlainMsg().contains("@")) {
            if (msgEvent.getPlainMsg().matches("落樱之都")) {
                menu();
                return true;

            } else if (msgEvent.getPlainMsg().matches("更新日志")) {
                msgEvent.send("目前只是做了个框架= =，需要各位大力支持，有意做数据策划的，可以私戳萌泪\n" +
                        "副本开放落樱平原五大难度，欢迎前来挑战");
                return true;
            } else if (msgEvent.getPlainMsg().matches("玩法")) {
                msgEvent.send("◇属性有效度◇\n" +
                ...
                return true;
            } else if (msgEvent.getPlainMsg().matches("注册.+")) {
                String name = msgEvent.getPlainMsg().substring(2).replaceAll("\\s", "");
                if (name.length() > 0) {
                    if (name.length() >= 10) {
                        name = name.substring(0, 10);
                        msgEvent.send(msgEvent.getSenderId(), "名字太长了，使用前十个字符~");
                    }
                    createRole(name);
                } else {
                    msgEvent.send(msgEvent.getSenderId(), "要有名字哦！");
                }
                return true;
            } else if (msgEvent.getPlainMsg().matches("改名.+")) {
                String name = msgEvent.getPlainMsg().substring(2).replaceAll("\\s", "");
                if (name.length() > 0) {
                    if (name.length() >= 10) {
                        name = name.substring(0, 10);
                        msgEvent.send(msgEvent.getSenderId(), "新名字太长了，使用前十个字符~");
                    }
                    rename(name);
                } else {
                    msgEvent.send(msgEvent.getSenderId(), "要有新名字哦！");
                }
                return true;
            } else if (msgEvent.getPlainMsg().matches("个人信息")) {
                showMyInfo();
                return true;
            } else if (msgEvent.getPlainMsg().matches("加经验[0-9]+")) {
                addExp(Integer.parseInt(msgEvent.getPlainMsg().substring(3)));
                return true;
            } else if (msgEvent.getPlainMsg().matches("嘤[0-9]+")) {
                addMoney(Integer.parseInt(msgEvent.getPlainMsg().substring(1)));
                return true;
            } else if (msgEvent.getPlainMsg().matches("恢复|回复")) {
                resetRole();
                return true;
            } else if (msgEvent.getPlainMsg().matches("休息")) {
                //resetRole();
                return true;
            } else if (msgEvent.getPlainMsg().matches("结束休息")) {
                //resetRole();
                return true;
            } else if (msgEvent.getPlainMsg().matches("加[0-9]+(力量|智力|体质|敏捷|魅力)")
                    || msgEvent.getPlainMsg().matches("加(力量|智力|体质|敏捷|魅力) *[0-9]+")) {
                int num = Integer.parseInt(msgEvent.getPlainMsg().split("\\D+")[0]);
                if (msgEvent.getPlainMsg().contains("力量")) {
                    addDian(DianType.LI, num);
                } else if (msgEvent.getPlainMsg().contains("智力")) {
                    addDian(DianType.ZHI, num);
                } else if (msgEvent.getPlainMsg().contains("体质")) {
                    addDian(DianType.TI, num);
                } else if (msgEvent.getPlainMsg().contains("敏捷")) {
                    addDian(DianType.MIN, num);
                } else if (msgEvent.getPlainMsg().contains("魅力")) {
                ...
                if (msgEvent.getTextMsg().matches("取消全部")) {
                    if (msgSource != Api.MsgSource.GROUP) {
                        return true;
                    }
                    // cancelAll();
                    return true;
                } else if (msgEvent.getTextMsg().matches("设置[A-Ca-c]面? *[1-5] +[0-9]+[Ww]?")) {
                    if (msgSource != Api.MsgSource.GROUP) {
                        return true;
                    }
                    return true;
            ...
            if (msgEvent.getAtId() == ERR_LONG || msgEvent.getMsgType() != MsgEvent.MsgType.GROUP) {
                return false;
            }
            /*
            if (msgEvent.getTextMsg().matches("加入@.+")) {
                //joinGuild(msgEvent.getAtId());
                return true;
            } else if (isBotAdmin(msgEvent.getSenderId())) {
                if (msgEvent.getTextMsg().matches("(删除|清除)全部@.+")) {
                    if (msgEvent.getAtId() == getRobotQQ()) {
                        // clear();
                        return true;
                    } else {
                        msgEvent.send(msgEvent.getSenderId(), "删除本群pcr信息需艾特Bot！");
                    }
                }
            }*/
        }
    ...
    private void createRole(String name) {
        Player player = SakuraUtils.getPlayer(msgEvent.getSenderId(), msgEvent);
        if (player == null) {
            player = new Player(msgEvent.getSenderId());
            player.setName(name);
            SakuraUtils.save(player);
            msgEvent.send(msgEvent.getSenderId(), "已创建角色【" + name + "】！");
        } else {
            msgEvent.send(msgEvent.getSenderId(), "已有角色，无法创建！");
        }
    }
    ...
    private void rename(String newName) {
        Player player = SakuraUtils.getPlayer(msgEvent.getSenderId(), msgEvent);
        if (player == null) {
            msgEvent.send(msgEvent.getSenderId(), "你还没注册呢！\n" +
                    "在落樱之都遨游需要通行证哦！\n" +
                    "据说发送【注册+昵称】就可以获得通行证辣~");
            return;
        }
        player.setName(newName);
        msgEvent.send(msgEvent.getSenderId(), "已更改昵称为" + newName);
        SakuraUtils.save(player);
    }
    ...
    private void showMyInfo() {
        ...
        msgEvent.send(msgEvent.getSenderId(), "-===\uD83C\uDF38落樱之都\uD83C\uDF38===-\n" +
    ...
    private void addExp(int num) {
        ...
        if (level != level2) {
            msgEvent.send(msgEvent.getSenderId(), "升至" + level2 + "级，\n当前经验：" + player.getExp() + "/" + player.getMaxExp());
        }
        SakuraUtils.save(player);
    }
    ...
    private void addMoney(int num) {
        ...
        player.addMoney(num);
        msgEvent.send(msgEvent.getSenderId(), "当前樱币：" + player.getMoney());
        SakuraUtils.save(player);
    }
    ...
    private void resetRole() {
        ...
        msgEvent.send(msgEvent.getSenderId(), "hp mp 已回满");
        SakuraUtils.save(player);
    }
    ...
    private void addDian(DianType type, int num) {
        ...
        if (num > player.getDian()) {
            msgEvent.send(msgEvent.getSenderId(), "剩余属性点" + player.getDian() + "，不足" + num);
            return;
        }
        switch (type) {
            case LI:
        ...
        msgEvent.send(msgEvent.getSenderId(), "加点完毕");
        SakuraUtils.save(player);
    }
}
