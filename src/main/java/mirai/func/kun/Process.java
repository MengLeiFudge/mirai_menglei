...
import static mirai.func.kun.KunUtils.getAdd;
import static mirai.func.kun.KunUtils.getBoss;
import static mirai.func.kun.KunUtils.getGrade;
import static mirai.func.kun.KunUtils.getNowSeason;
import static mirai.func.kun.KunUtils.getUser;
import static mirai.func.kun.KunUtils.getUserList;
import static mirai.func.kun.KunUtils.save;
import static mirai.func.kun.KunUtils.seasonSettlement;
import static mirai.func.kun.KunUtils.startNewSeason;
import static mirai.utils.BotAdminUtils.isBotAdmin;
import static mirai.utils.DateUtils.getDateStr;
import static mirai.utils.DateUtils.getFullTimeStr;
import static mirai.utils.DateUtils.getThisMonday;
import static mirai.utils.DateUtils.getTimeStr;
import static mirai.utils.DateUtils.milliSecondToStr;
import static mirai.utils.MyFileUtils.ERR_LONG;
import static mirai.utils.NickUtils.getNick;
import static mirai.utils.RandomUtils.getRandomDistributionDouble;
...
public class Process extends FuncProcess {
    ...
    public boolean process() {
        if (!msgEvent.getPlainMsg().contains("@")) {
            if (msgEvent.getPlainMsg().matches("养鲲|摸鲲|抓鲲|捕鲲")) {
                if (msgEvent.getMsgType() == MsgEvent.MsgType.GROUP) {
                    msgEvent.send(msgEvent.getSenderId(), "养鲲要私聊我哦！");
                    return true;
                }
                mk();
                return true;
            } else if (msgEvent.getPlainMsg().matches("属性")) {
                if (msgEvent.getMsgType() == MsgEvent.MsgType.GROUP) {
                    return true;
                }
                attribute();
                return true;
            } else if (msgEvent.getPlainMsg().matches("洗练.+[0-9]+")) {
                if (msgEvent.getMsgType() == MsgEvent.MsgType.GROUP) {
                    return true;
                }
                boolean[] wash = {msgEvent.getPlainMsg().contains("攻"), msgEvent.getPlainMsg().contains("防"), msgEvent.getPlainMsg().contains("血")};
                int num = Integer.parseInt(msgEvent.getPlainMsg().split("\\D+")[1]);
                washout(wash, num);
                return true;
            } else if (msgEvent.getPlainMsg().matches("挑战")) {
                if (msgEvent.getMsgType() == MsgEvent.MsgType.GROUP) {
                    return true;
                }
                attackBossInit();
                return true;
            } else if (msgEvent.getPlainMsg().matches("(查看|)[Bb]oss(属性|)")) {
                if (msgEvent.getMsgType() == MsgEvent.MsgType.GROUP) {
                    return true;
                }
                boss();
                return true;
            } else if (msgEvent.getPlainMsg().matches("等级排行(榜|)")) {
                levelRank();
                return true;
            } else if (msgEvent.getPlainMsg().matches("(财富|萌泪币|金钱)排行(榜|)")) {
                moneyRank();
                return true;
            } else if (msgEvent.getPlainMsg().matches("道具|背包")) {
                if (msgEvent.getMsgType() == MsgEvent.MsgType.GROUP) {
                    return true;
                }
                bag();
                return true;
            } else if (msgEvent.getPlainMsg().matches("命名.+")) {
                if (msgEvent.getMsgType() == MsgEvent.MsgType.GROUP) {
                    return true;
                }
                name(msgEvent.getPlainMsg().substring(2));
                return true;
            } else if (msgEvent.getPlainMsg().matches("商城")) {
                if (msgEvent.getMsgType() == MsgEvent.MsgType.GROUP) {
                    return true;
                }
                msgEvent.send("———— 商城 ————\n"
                        ...
                        + "查看卡：   100 /    80\n");
                return true;
            } else if (msgEvent.getPlainMsg().matches("(购买|买|出售|卖).+([0-9]+)?")) {
                if (msgEvent.getMsgType() == MsgEvent.MsgType.GROUP) {
                    return true;
                }
                boolean buy = msgEvent.getPlainMsg().startsWith("购买") || msgEvent.getPlainMsg().startsWith("买");
                int beginLen = msgEvent.getPlainMsg().startsWith("购买") || msgEvent.getPlainMsg().startsWith("出售") ? 2 : 1;
                int endLen;
                int num;
                if (msgEvent.getPlainMsg().matches(".+[0-9]+")) {
                ...
                String itemName = msgEvent.getPlainMsg().substring(beginLen, msgEvent.getPlainMsg().length() - endLen).trim();
                if (buy) {
                    buy(itemName, num);
                } else {
                    sell(itemName, num);
                ...
                return true;
            } else if (msgEvent.getPlainMsg().matches("签到")) {
                dailyAttendance();
                return true;
            } else if (msgEvent.getPlainMsg().matches("设置重置时间 *[0-9]+")) {
                if (msgEvent.getMsgType() == MsgEvent.MsgType.GROUP) {
                    return true;
                }
                int num = Integer.parseInt(msgEvent.getPlainMsg().split("\\D+")[1]);
                setResetTime(num);
                return true;
            } else if (msgEvent.getPlainMsg().matches("[开关]新赛季提示")) {
                if (msgEvent.getMsgType() == MsgEvent.MsgType.GROUP) {
                    return true;
                }
                boolean openNewSeasonTip = msgEvent.getPlainMsg().startsWith("开");
                setNewSeasonTip(openNewSeasonTip);
                return true;
            } else if (isBotAdmin(msgEvent.getSenderId())) {
                if (msgEvent.getPlainMsg().matches("(更改|修改).+[0-9]+")) {
                    int num = Integer.parseInt(msgEvent.getPlainMsg().split("\\D+")[1]);
                    String item = msgEvent.getPlainMsg().substring(2, (msgEvent.getPlainMsg().length() - (num + "").length())).trim();
                    change(item, num);
                    return true;
                } else if (msgEvent.getPlainMsg().matches("赠送全部 *[0-9]+")) {
                    int money = Integer.parseInt(msgEvent.getPlainMsg().split("\\D+")[1]);
                    giveMoneyToAll(money);
                }
            }
        } else {
            if (msgEvent.getAtId() == ERR_LONG) {
                return false;
            }
            if (msgEvent.getPlainMsg().matches("查看@.+")) {
                otherAttribute();
                return true;
            } else if (msgEvent.getPlainMsg().matches("进击@.+")) {
                attackOthers();
                return true;
            } else if (msgEvent.getPlainMsg().matches("赠送[0-9]+@.+")) {
                giveMoney(Integer.parseInt(msgEvent.getPlainMsg().substring(2, msgEvent.getPlainMsg().indexOf('@'))));
                return true;
            } else if (msgEvent.getPlainMsg().matches("@.+赠送[0-9]+")) {
                giveMoney(Integer.parseInt(msgEvent.getPlainMsg().substring(msgEvent.getPlainMsg().indexOf("赠送") + 2)));
                return true;
            }
        }
        return false;
    ...
    private void mk() {
        User user = getUser(msgEvent.getSenderId(), msgEvent);
        if (user == null) {
            user = new User(msgEvent.getSenderId());
            save(user);
            msgEvent.send(msgEvent.getSenderId(), "你孤身一人去无尽之海，发现一只小鲲崽遗弃在海岸边。\n" +
                    "你看它非常可爱，便给它取名为" + user.getName() +
                    "，决定抚养它成为最强的鲲王。\n" +
                    "只是这无尽之海非常广阔，你还需要抓更多的鲲喂给它，使其慢慢成长....");
            return;
        }
        long targetTime = user.mkTargetTime(msgEvent.getMilliTime());
        if (!isBotAdmin(msgEvent.getSenderId()) && targetTime != msgEvent.getMilliTime()) {
            msgEvent.send(msgEvent.getSenderId(), "下次摸鲲时间为" + getTimeStr(targetTime) +
                    "，还需等待" + milliSecondToStr(msgEvent.getMilliTime(), targetTime, true) + "~");
            return;
        }
        ...
        if (event < 10) {
            // 无事发生（0-9）
            msgEvent.send(msgEvent.getSenderId(), "你去无尽之海，结果什么都没摸到！\n"
                    + "只好带着" + name + "回去了。");
        } else if (event < 20) {
            // 被偷走钱（10-19）
            ...
            msgEvent.send(msgEvent.getSenderId(), "你去无尽之海，忽然一只巨鲲向你咬来！\n"
            ...
            if (event < 34) {
                // 鲲等级提高（30-33）
                msgEvent.send(msgEvent.getSenderId(), "你去无尽之海，抓了很多很多的鲲！\n"
                        + "你将大部分鲲喂给" + name + "，等级提高" + addLevel + "！\n"
                        + "剩余的鲲都被你卖掉，获得" + addMoney + "枚萌泪币！");
            } else if (event < 36) {
                // 鲲等级、萌泪币提高（34-35）
                msgEvent.send(msgEvent.getSenderId(), "你去无尽之海，不仅抓了很多鲲，还捞上来一个大宝箱！\n"
                        + "你将鲲喂给" + name + "，等级提高" + addLevel + "！\n"
                        + "你又打开宝箱，发现里面竟然有" + addMoney + "枚萌泪币！");
            } else if (event < 40) {
                // 萌泪币提高（36-39）
                msgEvent.send(msgEvent.getSenderId(), "你去无尽之海，抓了一些鲲，还捞上来一个小宝箱！\n"
                        + "你将鲲喂给" + name + "，等级提高" + addLevel + "！\n"
                        + "你又打开宝箱，发现里面竟然有" + addMoney + "枚萌泪币！");
            } else {
                // 正常（40-99）
                msgEvent.send(msgEvent.getSenderId(), "你去无尽之海，抓了一些鲲！\n"
                        + "你将大部分鲲喂给" + name + "，等级提高" + addLevel + "！\n"
                        + "剩余的鲲都被你卖掉，获得" + addMoney + "枚萌泪币！");
            }
        }
    ...
    private void attribute() {
        User user = getUser(msgEvent.getSenderId(), msgEvent);
        if (user == null) {
            msgEvent.send(msgEvent.getSenderId(), "在无尽之海，也许你会有所发现....\n指令提示：私聊【摸鲲】");
            return;
        }
        ...
        List<User> list = Rank.getListSortedByLevel();
        int myIndex = -1;
        int allNum = list.size();
        for (int i = 0; i < allNum; i++) {
            if (list.get(i).getQq() == msgEvent.getSenderId()) {
                myIndex = i + 1;
                break;
            }
        }
        ...
        msgEvent.send(msgEvent.getSenderId(), sendStr);
        save(user);
    }
    ...
    private void otherAttribute() {
        ...
        User atUser = getUser(msgEvent.getAtId(), msgEvent);
        if (atUser == null) {
            msgEvent.send(msgEvent.getSenderId(), "TA好像还没鲲呢！");
            return;
        }

        int ckk = user.getCkk();
        if (ckk <= 0) {
            msgEvent.send(msgEvent.getSenderId(), "TA的鲲似乎被一层迷雾笼罩....\n" + "指令提示：【购买查看卡】");
            return;
        }
        user.setCkk(ckk - 1);
        ...
        String defGrade = getGrade(def, level, 0.6, 0.9);
        String hpGrade = getGrade(hp, level, 4, 6);
        msgEvent.send(msgEvent.getSenderId(), "查看到对方信息如下：\n" +
                atUser.getName() + "\n" +
                "等级：" + level + "\n" +
                "血量：" + hp + "（" + hpGrade + "）\n" +
                "攻击：" + atk + "（" + atkGrade + "）\n" +
    ...
    private void washout(boolean[] type, int oneAttrUseNum) {
        ...
        boolean hp = type[2];
        if (!atk && !def && !hp) {
            msgEvent.send(msgEvent.getSenderId(), user.getName() + "一脸懵逼的看着你，不知道你要干什么。\n" +
                    "指令提示：攻防血至少洗练一项");
            save(user);
            return;
        }
        ...
        if (xlkNum < useNum) {
            msgEvent.send(msgEvent.getSenderId(), user.getName() + "也想洗练，但是你好像没有足够的洗练卡了QAQ\n" +
    ...
    private void attackOthers() {
        ...
        if (msgEvent.getAtId() == msgEvent.getSenderId()) {
            msgEvent.send(msgEvent.getSenderId(), user.getName() + "表示不想打自己，并向你丢了一个白眼！");
            return;
        }
        User atUser = getUser(msgEvent.getAtId(), msgEvent);
        if (atUser == null) {
        ...
        if (atLevel * 1.5 < myLevel) {
            msgEvent.send(msgEvent.getSenderId(), user.getName() + "实在是太强了，" +
                    atUser.getName() + "一看这气势，早就远远跑开了！");
            return;
        }
        long targetTime = user.jjTargetTime(msgEvent.getMilliTime());
        if (!isBotAdmin(msgEvent.getSenderId()) && targetTime != msgEvent.getMilliTime()) {
            msgEvent.send(msgEvent.getSenderId(), "下次进击时间为" + getTimeStr(targetTime) +
                    "，还需等待" + milliSecondToStr(msgEvent.getMilliTime(), targetTime, true) + "~");
            return;
        }
        ...
        myLevel += myAdd;
        atLevel += atAdd;
        msgEvent.send(msgEvent.getSenderId(), str + "经过磨炼，\n" +
                myName + "等级增加" + myAdd + "！\n" +
                atName + "等级增加" + atAdd + "！");
    ...
    private void attackBossInit() {
        ...
        if (ticket <= 0) {
            msgEvent.send(msgEvent.getSenderId(), "你还没挑战券呢！");
            return;
        }
        long targetTime = user.tzTargetTime(msgEvent.getMilliTime());
        if (!isBotAdmin(msgEvent.getSenderId()) && targetTime != msgEvent.getMilliTime()) {
            msgEvent.send(msgEvent.getSenderId(), "下次挑战时间为" + getTimeStr(targetTime) +
                    "，还需等待" + milliSecondToStr(msgEvent.getMilliTime(), targetTime, true) + "~");
            return;
        }
        user.setTzq(ticket - 1);
        Boss boss = getBoss(msgEvent);
        ...
        msgEvent.send(msgEvent.getSenderId(), str.append("获得了").append(getMoney).append("枚萌泪币！").toString());
        save(user);
        save(boss);
    }
    ...
    private void boss() {
        ...
        int ckk = user.getCkk();
        if (ckk <= 0) {
            msgEvent.send(msgEvent.getSenderId(), "你没有查看卡，无权查看对方信息！");
            return;
        }
        user.setCkk(ckk - 1);
        Boss boss = getBoss(msgEvent);
        ...
        msgEvent.send(msgEvent.getSenderId(), "Boss " + boss.getName() + "\n" +
                "攻击：" + boss.getAtk() + "\n" +
                "防御：" + boss.getDef() + "\n" +
                "剩余血量：" + hp);
        save(user);
    ...
    private void levelRank() {
        ...
        int size = list.size();
        if (size == 0) {
            msgEvent.send(msgEvent.getSenderId(), "当前无人上榜！\n（摸鲲、进击均可上榜）");
            return;
        }
        ...
        for (int i = 0; i < Math.min(size, 10); i++) {
            ...
            sb.append(i + 1).append(" Lv.").append(u.getLevel())
                    ...
                    .append(getNick(msgEvent.getGroupId(), u.getQq())).append("(").append(u.getQq()).append(")");
            if (j == 9) {
    ...
    private void moneyRank() {
        List<User> list = Rank.getListSortedByMoney();
        int size = list.size();
        if (size == 0) {
            msgEvent.send(msgEvent.getSenderId(), "当前无人上榜！\n（摸鲲、进击均可上榜）");
            return;
    ...
    private void bag() {
        ...
        msgEvent.send("改名卡：" + user.getGmk() + "张\n" +
                "洗练卡：" + user.getXlk() + "张\n" +
                "挑战券：" + user.getTzq() + "张\n" +
                "查看卡：" + user.getCkk() + "张\n" +
                "萌泪币：" + user.getMoney() + "枚");
    ...
    private void name(String newName) {
        ...
        if (newName.length() > 8) {
            msgEvent.send(msgEvent.getSenderId(), oldName + "表示这个名字实在是太长了，它根本记不住！\n" +
                    "指令提示：请使用8字符以内的名字");
            return;
        }
        if (newName.equals("")) {
            msgEvent.send(msgEvent.getSenderId(), "指令提示：命名xx");
            return;
        }
        int gmk = user.getGmk();
        if (gmk <= 0) {
            msgEvent.send(msgEvent.getSenderId(), oldName + "摇了摇头，表示不喜欢这个名字！\n" +
                    "指令提示：【购买改名卡】");
            return;
        }
        ...
        msgEvent.send(msgEvent.getSenderId(), oldName + "高兴地绕着你转了两圈，" +
                "看来它很喜欢这个名字！\n" +
                "以后它就叫" + newName + "啦！");
        save(user);
    }
    ...
    private void giveMoney(int giveMoney) {
        ...
        if (giveMoney > myMoney) {
            msgEvent.send(msgEvent.getSenderId(), "钱不够！\n你在想peach？");
            return;
        }

        user.subMoney(giveMoney);
        int fee = isBotAdmin(msgEvent.getSenderId()) ? 0 : (int) (giveMoney * getRandomDouble(0.05, 0.15));
        int add = giveMoney - fee;
        atUser.addMoney(add);
        msgEvent.send(msgEvent.getSenderId(), "收取手续费" + fee + "枚萌泪币，已赠送" + add + "枚萌泪币！");
        save(user);
        save(atUser);
    }
    ...
    private void giveMoneyToAll(int money) {
        ...
        msgEvent.send("已赠送所有人萌泪币" + money + "枚！");
    }
    ...
    private void buy(String itemName, int buyNum) {
        ...
        if (buyNum <= 0) {
            msgEvent.send(msgEvent.getSenderId(), "购买数量有误！");
            return;
        }
        int money = user.getMoney();
        int price;
        ...
        switch (itemName) {
            case "改名卡":
                itemNum = user.getGmk();
                if (itemNum + buyNum > 1) {
                    msgEvent.send(msgEvent.getSenderId(), "改名卡上限为1！");
                    return;
                }
                price = 18888;
                break;
            case "洗练卡":
                ...
                if (itemNum + buyNum > 9999) {
                    msgEvent.send(msgEvent.getSenderId(), "洗练卡上限为9999！");
                    return;
                }
                price = 30;
                break;
            case "挑战券":
                ...
                if (itemNum + buyNum > 99) {
                    msgEvent.send(msgEvent.getSenderId(), "挑战券上限为99！");
                    return;
                }
                price = 100;
                break;
            case "查看卡":
                itemNum = user.getCkk();
                if (itemNum + buyNum > 99) {
                    msgEvent.send(msgEvent.getSenderId(), "查看卡上限为99！");
                    return;
                }
                price = 100;
                break;
            default:
                msgEvent.send(msgEvent.getSenderId(), "[" + itemName + "]是神马东西？可以次吗？");
                return;
        }
        if (money < price * buyNum) {
            msgEvent.send(msgEvent.getSenderId(), "萌泪币：" + money + "\n不足" + price * buyNum + "，无法购买！");
            return;
        }
        switch (itemName) {
            case "改名卡":
        ...
        msgEvent.send(msgEvent.getSenderId(), "成功购买" + itemName + "×" + buyNum + "！\n" +
    ...
    private void sell(String itemName, int soldNum) {
        ...
        if (soldNum <= 0) {
            msgEvent.send(msgEvent.getSenderId(), "出售数量有误！");
            return;
        }
        int price;
        int itemNum;
        ...
        msgEvent.send(msgEvent.getSenderId(), "成功出售" + itemName + "×" + soldNum + "！\n" +
    ...
    private void dailyAttendance() {
        ...
        String today = getDateStr(msgEvent.getMilliTime());
        ...
        if (lastSignInDate.equals(today)) {
            msgEvent.send(msgEvent.getSenderId(), "共签到" + allTimes + "次\n" +
                    "本周已签到" + weekTimes + "天\n" +
                    "明天再来签到吧！");
            return;
        }
        ...
        String thisMonday = getThisMonday(msgEvent.getMilliTime());
    ...
    private void setResetTime(int resetTime) {
        ...
        if (user.getResetTime() == resetTime) {
            msgEvent.send(msgEvent.getSenderId(), "当前重置时间是" + resetTime + "时，无需更改！");
            return;
        }
        long lastSetTime = user.getLastSetTime();
        if (msgEvent.getMilliTime() - lastSetTime < 604800000L) {
            msgEvent.send(msgEvent.getSenderId(), "距上一次设置重置时间不足一周！\n请于"
                    + getFullTimeStr(lastSetTime + 604800000L) + "后再试！");
            return;
        }
        user.setResetTime(resetTime);
        user.setLastSetTime(msgEvent.getMilliTime());
        msgEvent.send(msgEvent.getSenderId(), "当前重置时间已更改为" + resetTime + "时！");
        save(user);
    }
    ...
    private void setNewSeasonTip(boolean open) {
        ...
        msgEvent.send(msgEvent.getSenderId(), "已" + (open ? "打开" : "关闭") + "赛季提示！");
        save(user);
    }
    ...
    private void change(String key, int newValue) {
        ...
        switch (key) {
            ...
            case "等级":
                ...
                break;
            default:
                msgEvent.send(msgEvent.getSenderId(), "错误的修改目标 [" + key + "]");
                return;
        }
        msgEvent.send(msgEvent.getSenderId(), "已修改" + key + "为" + newValue + "！");
        save(user);
    }
}
